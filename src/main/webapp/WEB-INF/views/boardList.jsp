	...
	<div class="container-fluid">
				...
				<c:forEach var="i" items="${boardList.getContent()}">
					<tr <c:if test="${i.isNotice == true}">class="table-secondary"</c:if>>
						...
						<td><c:choose>
								...
								<c:otherwise>
									<c:out value="${i.boardNo}" />
								</c:otherwise>
							</c:choose></td>
						<td><c:choose>
								<c:when test="${i.isNotice == true}">
									<b><a href="${link }"><c:out value="${i.boardTitle}" /></a></b>
								</c:when>
								<c:otherwise>
									<a href="${link}"><c:out value="${i.boardTitle}" /></a>
								</c:otherwise>
							</c:choose></td>
						<td><c:out value="${i.memberId}" /></td>
						<td><c:out value="${i.boardHit}" /></td>
		...
		<div class="d-flex justify-content-center">
			<select id="select" class="form-select mx-2" style="width: 100px">
				...
				<option>내용</option>
				<option>작성자</option>
			</select> <input type="text" id="searchText" class="d-inline form-control mx-2" style="width: 200px" name="searchText" placeholder="검색어를 입력하세요."> <input type="button" id="searchBtn" class="d-inline btn btn-secondary mx-2" name="searchBtn" value="검색">
		</div>
